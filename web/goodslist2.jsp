...
<div id="top">
    <div id="navtop">
        ...
        <div class="ntright">
            ...
            <div><a href="/xaiomi/shopcart.jsp">购物车 (0)</a></div>
        </div>
    </div>
</div>
<header>
...
<div class="listcontainer">
    ...
    <div class="glist">
        ...
        <div class="glistbox" id="search_area">
            <!-- 搜索到的商品展示区域 -->
...
<script>
    $(document).ready(function(){
        let $searchWord = window.location.search.split("?")[1].split("&");
        // 此时获得的？后面的参数是 {level=1,goodsTypeId=102}
        console.log($searchWord);
        // 获取参数值
        let $level = $searchWord[0].split("=")[1];
        let $goodsTypeId = $searchWord[1].split("=")[1];
        console.log($level, $goodsTypeId);
        $.ajax({
            url: "/xaiomi/u/search/" + $level+"/"+$goodsTypeId ,
            method: "get",
            success: function (response) {
                ...
                let $goodslist = response.objectMap.listGoods;
                loadGoods($goodslist, $('#search_area'));
            },
            error: function (response) {
                console.log("请求迷路了~");
            }
        });
        // 此方法加载商品列表， 拼接各种 div，样式
        function loadGoods(goodslist, container) {
            for(let i = 0; i < goodslist.length; i++) {
                let $boxitem = $("<div>").addClass("goodsboxitem");
                let $a = $("<a>").attr("href","/xaiomi/detail.jsp?goodsId="+goodslist[i].id).text("商品详情");
                ...
                let $desc = $("<desc>").addClass("desc").text(goodslist[i].remark.substring(10));
                let $price = $("<div>").addClass("price");
                let $span = $("<span>").text(goodslist[i].price + "元");
                let $del = $("<del>").text(goodslist[i].price + "元");
                let $thumbnailx = $("<div>").addClass("thumbnailx");
                let $img2 = $("<div>").attr("src", goodslist[i].goodsImages[1].path)
                $thumbnailx.append($img2);

                $price.append($span).append($del);
                $content.append($thumb).append($title).append($desc).append($price).append($thumbnailx);
                $a.append($content);
                $boxitem.append($a);
