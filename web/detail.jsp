...
<script>
    $(document).ready(function(){
        ...
        console.log($goodsId);
        $.ajax({
            url: "/xaiomi/goods/detail/"+$goodsId,
            method: "get",
            success: function(response){
                console.log(response);
                let $goodsDetail = response.objectMap.goodsDetail;
                $('#goodsId').val($goodsDetail.id);
                $("#goods-name").text($goodsDetail.name);
                $("#goods-desc").text($goodsDetail.remark);
                $("#goods-price span").text($goodsDetail.price+"元");
                $("#goods-price del").text($goodsDetail.price+"元");
                $("#goods-price del").text($goodsDetail.price+"元");
                $("#goods-subtotal").text($goodsDetail.price+"元");
                $("#dtleft >img").attr("src",$goodsDetail.goodsImages[0].path);
            },
        ...
        $('#addCart').click(function () {
            ...
            $.ajax({
                url: "/xaiomi/shopcart/add/" + $goodsId,
                method: "get",
                success: function (response) {
